...
public class UpcomingWeekService extends IntentService {
    ...
    DatabaseReference db = FirebaseDatabase.getInstance().getReference().child("students").child(auth.getCurrentUser().getUid()).child("UpcomingWeek");

    public UpcomingWeekService() {
        super("UpcomingWeekService");
    }
                        ...
                        public void onResponse(JSONObject response) {
                            try {
                                ...
                                while (c<response.getJSONArray("messStatus").length()){
                                    ...
                                    map.put("ls",lnch);
                                    map.put("ds",dnnr);
                                    db.child(String.valueOf(c)).updateChildren(map);
                                    c++;
                                }
                                int n = 0;
                                while (n<response.getJSONArray("extraData").length()){
                                    ...
                                    Map<String,Object> map = new HashMap<>();
                                    map.put("date",date);
                                    db.child(String.valueOf(n)).updateChildren(map);
                                    n++;
                                }
